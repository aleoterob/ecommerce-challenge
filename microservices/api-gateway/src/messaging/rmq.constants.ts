export const AUTH_SERVICE_CLIENT = 'AUTH_SERVICE_CLIENT';
export const CATALOG_SERVICE_CLIENT = 'CATALOG_SERVICE_CLIENT';
export const INVENTORY_SERVICE_CLIENT = 'INVENTORY_SERVICE_CLIENT';

export const AUTH_REGISTER_PATTERN = 'auth.register';
export const AUTH_LOGIN_PATTERN = 'auth.login';
export const AUTH_PROFILE_PATTERN = 'auth.profile';

export const CATALOG_CREATE_PRODUCT_PATTERN = 'catalog.create-product';
export const CATALOG_LIST_PRODUCTS_PATTERN = 'catalog.list-products';
export const CATALOG_GET_PRODUCT_PATTERN = 'catalog.get-product';

export const INVENTORY_ADJUST_STOCK_PATTERN = 'inventory.adjust-stock';
